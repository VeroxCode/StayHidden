...
function this:tweenSpeed(PlayerId, TweenTime, Speed, reverse, easing)
...
function this:getSpeed(PlayerId)

	local Player = Players:GetPlayerByUserId(PlayerId)
	local MatchPlayer = game.ReplicatedStorage.Match.Players[Player.Name]

	if (Player ~= nil) then
		return MatchPlayer.Values:GetAttribute("Speed")
	end
end
...
function this:setDefault(Speed)
	this.Speeds.Hunter.Default = Speed
end

function this:getDefault()
	return this.Speeds.Hunter.Default
end
...
function this:startRunner()
	...
	Runner = Events.Game.ServerTick.Event:Connect(function(delta)
		...
		for index, MatchPlayer in pairs(MatchPlayers) do
			...
			local defaultSpeed = this.Speeds[Role].Default
			...
			MatchPlayer.Values:SetAttribute("Speed", calcSpeed)
			MatchPlayer.Values:SetAttribute("SpeedMultiplier", calcMultiplier)
			this:tweenSpeed(GameFS:getUserId(MatchPlayer), 0.25, finalSpeed, false, Enum.EasingStyle.Linear)
